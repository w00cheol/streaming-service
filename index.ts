...
// let inputFile: string = "./../../Media/Azure_Bumper.mp4";
...
var inputUrl: string;
// let inputUrl: string = "./../../Media/TearsOfSteel_AAC_und_ch2_56kbps.mp4";
...
export async function main(url: string) {
  ...
  try {
    ...
    let input = await getJobInputType(uniqueness, inputUrl);
...
async function getJobInputType(uniqueness: string, url: string): Promise<JobInputUnion> {
  ...
  if (paths.streamingPaths) {
    ...
    paths.streamingPaths.forEach(path => {
      path.paths?.forEach(formatPath => {
        manifestPath = "https://endpoint-ams-" + streamingEndpoint.hostName + formatPath
